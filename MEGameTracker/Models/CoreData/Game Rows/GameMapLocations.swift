...
extension MapLocation {
    ...
    public static func getAllMissions(
        ...
        return Mission.getAllFromData(with: manager) { fetchRequest in
            if let gameVersion = gameVersion {
                fetchRequest.predicate = NSPredicate(format: "(inMapId = %@ AND gameVersion = %@)", inMapId, gameVersion.stringValue)
            } else {
                fetchRequest.predicate = NSPredicate(format: "(inMapId = %@)", inMapId)
            }
        }.filter({ !$0.isHidden }).map{ $0 as MapLocationable }
    ...
    public static func getAllMaps(
        ...
        return Map.getAllFromData(gameVersion: gameVersion, with: manager) { fetchRequest in
            fetchRequest.predicate = NSPredicate(format: "(name CONTAINS[cd] %@ AND isShowInList = true)", name)
    ...
    public static func getAllMissions(
        ...
        return Mission.getAllFromData(with: manager) { fetchRequest in
            if let gameVersion = gameVersion {
                fetchRequest.predicate = NSPredicate(format: "(name CONTAINS[cd] %@ AND gameVersion = %@ AND isShowInList = true)", name, gameVersion.stringValue)
            } else {
                fetchRequest.predicate = NSPredicate(format: "(name CONTAINS[cd] %@ AND isShowInList = true)", name)
            }
            fetchRequest.fetchLimit = limit
    ...
    public static func addChildMapLocations(mapLocations: [MapLocationable]) -> [MapLocationable] {
        ...
        for mapLocation in mapLocations {
            ...
            if mapLocation.mapLocationType == .map {
                childMapLocations += MapLocation.getAllMissions(inMapId: mapLocation.id, gameVersion: App.current.gameVersion)
                childMapLocations += MapLocation.getAllItems(inMapId: mapLocation.id, gameVersion: App.current.gameVersion)
