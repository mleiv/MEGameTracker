...
// MARK: Save
    ...
    public mutating func save(
        ...
        if isCascadeChanges != .none && isCascadeChanges != .up && !GamesDataBackup.current.isSyncing {
            isSaved = isSaved && (shepard?.saveAnyChanges(isCascadeChanges: .down, isAllowDelay: false, with: manager) ?? false)
        }
...
// MARK: Delete
    ...
    public static func notifyDeleteToCloud(
        ...
        }.map { return DeletedRow(source: Decision.entityName, identifier: Decision.getIdentifyingName(id: $0.id, gameSequenceUuid: $0.gameSequenceUuid)) }
        
        deletedRows += getAllIdentifiers(ofType: Event.self, with: manager) { fetchRequest in
            fetchRequest.predicate = NSPredicate(format: "(gameSequenceUuid = %@)", uuid)
        }.map { return DeletedRow(source: Event.entityName, identifier: Event.getIdentifyingName(id: $0.id, gameSequenceUuid: $0.gameSequenceUuid)) }
        
        deletedRows += getAllIdentifiers(ofType: Item.self, with: manager) { fetchRequest in
            fetchRequest.predicate = NSPredicate(format: "(gameSequenceUuid = %@)", uuid)
        }.map { return DeletedRow(source: Item.entityName, identifier: Item.getIdentifyingName(id: $0.id, gameSequenceUuid: $0.gameSequenceUuid)) }
        
        deletedRows += getAllIdentifiers(ofType: Map.self, with: manager) { fetchRequest in
            fetchRequest.predicate = NSPredicate(format: "(gameSequenceUuid = %@)", uuid)
        }.map { return DeletedRow(source: Map.entityName, identifier: Map.getIdentifyingName(id: $0.id, gameSequenceUuid: $0.gameSequenceUuid)) }
        
        deletedRows += getAllIdentifiers(ofType: Mission.self, with: manager) { fetchRequest in
            fetchRequest.predicate = NSPredicate(format: "(gameSequenceUuid = %@)", uuid)
        }.map { return DeletedRow(source: Mission.entityName, identifier: Mission.getIdentifyingName(id: $0.id, gameSequenceUuid: $0.gameSequenceUuid)) }
        
        deletedRows += getAllIdentifiers(ofType: Note.self, with: manager) { fetchRequest in
            fetchRequest.predicate = NSPredicate(format: "(gameSequenceUuid = %@)", uuid)
        }.map { return DeletedRow(source: Note.entityName, identifier: Note.getIdentifyingName(id: $0.id, gameSequenceUuid: $0.gameSequenceUuid)) }
        
        deletedRows += getAllIdentifiers(ofType: Person.self, with: manager) { fetchRequest in
            fetchRequest.predicate = NSPredicate(format: "(gameSequenceUuid = %@)", uuid)
        }.map { return DeletedRow(source: Person.entityName, identifier: Person.getIdentifyingName(id: $0.id, gameSequenceUuid: $0.gameSequenceUuid)) }
        
        deletedRows += getAllIdentifiers(ofType: Shepard.self, with: manager) { fetchRequest in
            fetchRequest.predicate = NSPredicate(format: "(gameSequenceUuid = %@)", uuid)
        }.map { return DeletedRow(source: Shepard.entityName, identifier: Shepard.getIdentifyingName(id: $0.id, gameSequenceUuid: $0.gameSequenceUuid)) }
        ...
        autoreleasepool {
            if let row: GameSequences = manager.getOne(alterFetchRequest: { fetchRequest in
                    fetchRequest.sortDescriptors = [NSSortDescriptor(key:
                        #keyPath(GameSequences.modifiedDate), ascending: false)]
                }),
