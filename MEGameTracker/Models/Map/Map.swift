...
// MARK: Computed Properties
    ...
    public func isAvailableInGame(_ gameVersion: GameVersion) -> Bool {
        return generalData.isAvailable && events.filter({ (e: Event) in return e.type == .unavailableInGame ? e.isBlockingInGame(gameVersion) : false }).isEmpty
    }
    ...
    fileprivate mutating func applyToHierarchy(
        ...
        if isCascadeChanges != .down, var parentMap = self.parentMap, parentMap.isExplorable {
            ...
            if !isExplored && parentMap.isExplored {
                ...
                let exploredCount = siblingMaps.filter({ $0 == self }).filter({ $0.isExplorable && $0.isExplored }).count + 1
...
extension Map: SerializedDataRetrievable {
    ...
    public init?(data: SerializableData?) {
        ...
        self.init(id: id, gameSequenceUuid: gameSequenceUuid, gameVersion: gameVersion, generalData: dataMap, data: data)
    }
...
extension Map {
    ...
    static func sort(_ a: Map, b: Map) -> Bool {
        if a.inMapId == nil && b.inMapId != nil {
            return true
        } else if a.isExplored != b.isExplored {
            return b.isExplored // push to end
        } else if a.isAvailable != b.isAvailable {
            return a.isAvailable
        } else {
            return a.name < b.name // still unsure - all the others sort by id.
        }
    }
}
