...
public enum MapLocationType: Int {
    ...
    public init?(stringValue: String) {
        guard let type = MapLocationType.stringValues.filter({ $0.1 == stringValue }).flatMap({ $0.0 }).first else { return nil }
        self = type
    }
    ...
    public init?(headingValue: String) {
        guard let type = MapLocationType.headingValues.filter({ $0.1 == headingValue }).flatMap({ $0.0 }).first else { return nil }
        self = type
    }
