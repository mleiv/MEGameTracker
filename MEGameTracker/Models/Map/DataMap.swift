...
// MARK: Initialization
    ...
    internal mutating func setGameVersionData() {
        ...
        mapType = MapType(stringValue: gameVersionData["mapType"]?.string ?? rawGeneralData["mapType"]?.string) ?? mapType
        ...
        isShowInList = gameVersionData["isShowInList"]?.bool ?? (rawGeneralData["isShowInList"]?.bool ?? isShowInList)
        isOpensDetail = gameVersionData["isOpensDetail"]?.bool ?? (rawGeneralData["isOpensDetail"]?.bool ?? isOpensDetail)
        ...
        if let sizeString = referenceSize {
            ...
            if referenceSizeBits.count == 2 {
                if let w = NumberFormatter().number(from: referenceSizeBits[0]), let h = NumberFormatter().number(from: referenceSizeBits[1]) {
...
//        (rawGeneralData["unavailabilityMessages"]?.array ?? []).forEach{ if let message = $0.string { _unavailabilityMessages.append(message) } }
...
//        }
        ...
        annotationNote = gameVersionData["annotationNote"]?.string ?? (rawGeneralData["annotationNote"]?.string ?? annotationNote)
        ...
        rerootBreadcrumbs = gameVersionData["rerootBreadcrumbs"]?.bool ?? (rawGeneralData["rerootBreadcrumbs"]?.bool ?? rerootBreadcrumbs)
...
extension DataMap {
    ...
    public func getBreadcrumbs() -> [AbbreviatedMapData] {
        if isDummyData {
            return [AbbreviatedMapData(id: "0a", name: "Galaxy Map"), AbbreviatedMapData(id: "0b", name: "Omega Nebula")]
        }
...
//            return true
//        }
//        if isHidden && gameVersionData[gameVersion.stringValue]?["isHidden"]?.bool == true {
//            return true
//        }
//        return false
//        // we don't use events in maps
//        //events.filter({ (e: Event) in return e.type == .unavailableInGame ? e.isBlockingInGame(gameVersion) : false }).isEmpty
//    }
}
