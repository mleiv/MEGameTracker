...
extension ConversationRewardSet {
    ...
    public func flatRows(level: Int = 0) -> [ConversationRewards.FlatData] {
        if let point = self.point {
            let points = point.type == .paragade ? point.paragadeValue.map({ "\($0)" }).joined(separator: "/") : "\(point.value)"
            let options: [ConversationRewards.FlatDataOption] = [
                (id: point.id, type: point.type, points: points, context: point.context, trigger: point.trigger, isSelected: point.isSelected)
            ]
            ...
            if isExclusiveSet && hasOnlyOneLevel {
                ...
                let nextOptions = nextLevelOptions.flatMap({ $0.options.first }).map({
                    return (id: $0.id, type: $0.type, points: $0.points, context: $0.context, trigger: $0.trigger, isSelected: $0.isSelected) as ConversationRewards.FlatDataOption
                })
    ...
    public func selectedIds() -> [String] {
        ...
        for point in (subset ?? []) {
            selectedIds = selectedIds + point.selectedIds()
        }
