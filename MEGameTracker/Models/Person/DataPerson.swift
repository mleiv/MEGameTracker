...
// MARK: Initialization
    ...
    internal mutating func setGameVersionData() {
        ...
        personType = PersonType(stringValue: gameVersionData["personType"]?.string) ?? (PersonType(stringValue: rawGeneralData["personType"]?.string) ?? personType)
        ...
        if let photo = Photo(filePath: gameVersionData["photo"]?.string) ?? Photo(filePath: rawGeneralData["photo"]?.string) {
            self.photo = photo
        }
    }
