...
public enum PersonType: String {
    ...
    public init?(stringValue: String?) {
        guard let type = PersonType.stringValues.filter({ $0.1 == stringValue }).flatMap({ $0.0 }).first else { return nil }
        self = type
    }
    ...
    public init?(headingValue: String?) {
        guard let type = PersonType.headingValues.filter({ $0.1 == headingValue }).flatMap({ $0.0 }).first else { return nil }
        self = type
    }
