		...
		public static func formatCode(_ code: String!, lastCode: String! = nil) -> String {
			...
			if lastCode != nil {
				...
				if requestedSubtractChars > 0 && actualSubtractChars < requestedSubtractChars {
					...
					unformattedCode = subtractChars >= unformattedCode.length
						? ""
						: unformattedCode.stringFrom(0, to: -1 * subtractChars)
				}
			}
			//add formatting
			var formattedCode = unformattedCode.replacingOccurrences(of: "([^\\.]{3})",
				with: "$1.",
