...
final public class GamesDataBackup: SimpleCloudKitManageable {
    ...
    public func changesToCloudCompletion(
        ...
        log("changesToCloudCompletion isSuccess = \(isSuccess) savedRecords = \(savedRecords.count) deletedRecordIds = \(deletedRecordIds)")
    ...
    public var isUseCloud: Bool {
        get { return _isUseCloud.get() ?? true }
        set { return _isUseCloud.set(newValue) }
    }
    public var isFirstSync: Bool {
        get { return _isFirstSync.get() ?? false }
        set { return _isFirstSync.set(newValue) }
    }
    public var isCloudQuotaExceeded: Bool {
        get { return _isCloudQuotaExceeded.get() ?? false }
        set { return _isCloudQuotaExceeded.set(newValue) }
    }
    public var accountName: String? {
        get { return _accountName.get() }
        set { return _accountName.set(newValue) }
    }
    public var changeToken: CKServerChangeToken? {
        get { return _changeToken.get() }
        set { return _changeToken.set(newValue) }
    }
    
    // Specific to GameDataBackup
    
    var _isUseCloud = SimpleUserDefaults<Bool>(name: "IsUseGameDataCloud", defaultValue: true)
    var _isFirstSync = SimpleUserDefaults<Bool>(name: "IsFirstGameDataSync", defaultValue: true)
    var _isCloudQuotaExceeded = SimpleUserDefaults<Bool>(name: "IsCloudQuotaExceeded", defaultValue: false)
    var _accountName = SimpleUserDefaults<String>(name: "GameDataAccountName")
    var _changeToken = SimpleUserDefaults<CKServerChangeToken>(name: "GameDataChangeToken")
    ...
    public func saveAllChangesFromCloud() -> Bool {
        log("saveAllChangesFromCloud gameChanges = \(gameChanges.count) shepardChanges = \(shepardChanges.count) mapChanges = \(mapChanges.count) missionChanges = \(missionChanges.count) personChanges = \(personChanges.count) decisionChanges = \(decisionChanges.count) itemChanges = \(itemChanges.count) noteChanges = \(noteChanges.count) eventChanges = \(eventChanges.count)")
    ...
    public func confirmAllChangesToCloud (
        ...
        // There is a small risk that we will delete changes made between the time of the post and the response. I am not dealing with that yet.
        // There is a larger risk that another copy of the object currently open will re-save the pending changes back to core data. Which will just result in it being saved twice, not a hugely dangerous thing.
        
        let isConfirmed = true // we really don't need to halt anything if this breaks - it will just resend later.
        
        let decisionIdentifiers = savedRecords.filter{ $0.recordType == Decision.cloudRecordType }.flatMap{ $0.recordID.recordName }
        ...
        let eventIdentifiers = savedRecords.filter{ $0.recordType == Event.cloudRecordType }.flatMap{ $0.recordID.recordName }
        Event.getAll(identifiers: eventIdentifiers, with: manager).forEach {
            var e = $0; e.pendingCloudChanges = nil; _ = e.save(with: manager)
        }
        let itemIdentifiers = savedRecords.filter{ $0.recordType == Item.cloudRecordType }.flatMap{ $0.recordID.recordName }
        Item.getAll(identifiers: itemIdentifiers, with: manager).forEach {
            var i = $0; i.pendingCloudChanges = nil; _ = i.save(with: manager)
        }
        let mapIdentifiers = savedRecords.filter{ $0.recordType == Map.cloudRecordType }.flatMap{ $0.recordID.recordName }
        Map.getAll(identifiers: mapIdentifiers, with: manager).forEach {
            var m = $0; m.pendingCloudChanges = nil; _ = m.save(with: manager)
        }
        let missionIdentifiers = savedRecords.filter{ $0.recordType == Mission.cloudRecordType }.flatMap{ $0.recordID.recordName }
        Mission.getAll(identifiers: missionIdentifiers, with: manager).forEach {
            var m = $0; m.pendingCloudChanges = nil; _ = m.save(with: manager)
        }
        let noteIdentifiers = savedRecords.filter{ $0.recordType == Note.cloudRecordType }.flatMap{ $0.recordID.recordName }
        Note.getAll(identifiers: noteIdentifiers, with: manager).forEach {
            var n = $0; n.pendingCloudChanges = nil; _ = n.save(with: manager)
        }
        let personIdentifiers = savedRecords.filter{ $0.recordType == Person.cloudRecordType }.flatMap{ $0.recordID.recordName }
        Person.getAll(identifiers: personIdentifiers, with: manager).forEach {
            var p = $0; p.pendingCloudChanges = nil; _ = p.save(with: manager)
        }
        let gameIdentifiers = savedRecords.filter{ $0.recordType == GameSequence.cloudRecordType }.flatMap{ $0.recordID.recordName }
        ...
        let shepardIdentifiers = savedRecords.filter{ $0.recordType == Shepard.cloudRecordType }.flatMap{ $0.recordID.recordName }
        ...
        log("confirmAllChangesToCloud decisionIdentifiers = \(decisionIdentifiers.count) eventIdentifiers = \(eventIdentifiers.count) itemIdentifiers = \(itemIdentifiers.count) mapIdentifiers = \(mapIdentifiers.count) missionIdentifiers = \(missionIdentifiers.count) noteIdentifiers = \(noteIdentifiers.count) personIdentifiers = \(personIdentifiers.count) gameIdentifiers = \(gameIdentifiers.count) shepardIdentifiers = \(shepardIdentifiers.count) deletedRecordIds = \(deletedRecordIds.count)")
