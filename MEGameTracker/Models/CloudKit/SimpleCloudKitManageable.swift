...
extension SimpleCloudKitManageable {
    ...
    public func shouldSync() -> Bool {
        ...
        // - already syncing
        log("shouldSync? isUseCloud = \(isUseCloud) appState = \(appState) isPendingCloudChanges = \(isPendingCloudChanges) isSyncing = \(isSyncing)")
        return isUseCloud
    ...
    public func sync(
        ...
        sync(appState: self.appState, isPartialPost: isPartialPost, qualityOfService: nil, completion: { _ in })
    }
    ...
    public func sync(
        ...
        sync(appState: appState ?? self.appState, isPartialPost: false, qualityOfService: qualityOfService, completion: completion)
    }
    public func sync(
        appState: AppStateForCloud,
        isPartialPost: Bool,
        ...
        log("Starting sync() appState = \(appState) isPartialPost = \(isPartialPost) qualityOfService = \(qualityOfService) oldQualityOfService = \(oldQualityOfService)")
        isSyncing = true
        cachedCloudImages = [:]
    ...
    func firstTimeSync(
        ...
        postChanges() { (isPosted: Bool) in
            if isPosted {
                // get all data from cloud
                self.fetchChanges() { (isFetched: Bool) in
    ...
    func subsequentSync(
        ...
        fetchChanges() { (isFetched: Bool) in
            if isFetched {
                ...
                self.postChanges() { (isPosted: Bool) in
                    completion(isPosted)
                }
            } else {
                completion(false)
    ...
    public func shouldFetchChanges() -> Bool {
        ...
        log("shouldFetchChanges? isUseCloud = \(isUseCloud) accountStatus = \(accountStatus) isFetchingChanges = \(isFetchingChanges)")
        return isUseCloud
...
//            && appState != .stop // NO! this state is processed separately, so it doesn't block subsequent actions
    ...
    public func fetchChanges(
        ...
        operation.recordChangedBlock = { (record: CKRecord) -> Void in
            self.saveOneFromCloud(record: record) {
                (isQueued: Bool) in
        ...
        operation.recordWithIDWasDeletedBlock = { (recordId: CKRecordID, recordType: String) -> Void in
            if !isFirstSync {
                ...
                self.deleteOneFromCloud(recordId: recordId, recordType: recordType) {
                    (isQueued: Bool) in
                    isSuccess = isSuccess && isQueued
                }
            }
        ...
        operation.completionBlock = {
            ...
            completion(isFinished)//isSuccess)// ignore local save from cloud failures for now - proceed to saving local changes to cloud
        }
    ...
    public func shouldPostChanges() -> Bool {
        ...
        log("shouldPostChanges? isUseCloud = \(isUseCloud) accountStatus = \(accountStatus) appState = \(appState) isCloudQuotaExceeded = \(isCloudQuotaExceeded) isPendingCloudChanges = \(isPendingCloudChanges) time since lastPostedDate = \(Date().timeIntervalSince(lastPostedDate)) postChangeWaitIntervalSeconds = \(postChangeWaitIntervalSeconds) isPostingChanges = \(isPostingChanges)")
        return isUseCloud
            && accountStatus == .available
    ...
    public func postChangeBatch(
        ...
        log("postChangeBatch recordsToSave = \(recordsToSave.count) recordIDsToDelete = \(recordIDsToDelete.count) isRootRequest = \(isRootRequest)")
            ...
            if let error = error {
                ...
                if (error as? CKError)?.code == CKError.quotaExceeded {
                    ...
                    if !self.isCloudQuotaExceeded {
                        ...
                        let alert = Alert(title: "Cloud Error", description: "You have reached your cloud quota. Information will continue storing locally, but cloud data will be limited.")
    ...
    internal func processChangeBatchLimitExceeded(
        ...
        log("processChangeBatchLimitExceeded recordsToSave = \(recordsToSave.count) recordIDsToDelete = \(recordIDsToDelete.count)")
...
//        }
    ...
    public func switchAccountAlert(
        ...
        var alert = Alert(title: "Cloud Account Changed", description: "The games saved to this device belong to another cloud account. What do you want to do? Copy everything over to the new cloud account, or replace everything with the new cloud account (data may be lost)?")
