...
extension GameSequence: CloudDataStorable {
    ...
    public static func saveOneFromCloud(
        ...
        if let recordId = changeSet["recordId"]?.string,
            ...
            if let lastPlayedShepardUuid = pendingData?["lastPlayedShepard"]?.string ?? changeSet["lastPlayedShepard"]?.string {
                game.shepard = Shepard(uuid: lastPlayedShepardUuid, gameSequenceUuid: uuid) // dummy copy, just to keep ref to shepardUuid
            }
