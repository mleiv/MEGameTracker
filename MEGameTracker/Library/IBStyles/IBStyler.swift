...
/// Applies IBStyles to IBStylable elements. It does not auto-detect when and how to do this, alas, so each IBStylable element has to call home to it in order to trigger events. Call applyStyles() inside IBStylable's prepareForInterfaceBuilder() or layoutSubviews().
public class IBStyler: NSObject {
    ...
    public func applyStyles() {
        ...
        IBStyleManager.current.apply(identifier: elementIdentifier, to: styledElement)
    ...
    public func applyState(_ state:UIControl.State) {
        ...
        IBStyleManager.current.apply(identifier: elementIdentifier, to: styledElement, forState: state)
    }
