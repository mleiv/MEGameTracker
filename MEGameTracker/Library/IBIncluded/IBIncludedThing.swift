...
/// Allows for removing individual scene design from the flow storyboards 
/// and into individual per-controller storyboards, which minimizes git merge conflicts.
///
...
open class IBIncludedThing: UIViewController, IBIncludedThingLoadable {
	...
	open override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
		...
		// This can cause unexpected multiple calls to prepareForSegue, 
		//   so I prefer to be more cautious about which view controllers invoke prepareForSegue.
		// See IBIncludedThingDemo Fourth/SixthController for examples/options with embedded view controllers.
...
/// Because UIViewController does not preview in Interface Builder, 
///	this is an Interface-Builder-only companion to IBIncludedThing. 
/// Typically you would set the scene owner to IBIncludedThing 
///	and then set the top-level view to IBIncludedThingPreview, with identical IBInspectable values.
@IBDesignable
...
/// Can be used identically to IBIncludedThing, but for subviews inside scenes rather than an entire scene.
/// The major difference is that IBIncludedSubThing views, unfortunately, cannot be configured using prepareForSegue, 
///	 because they are loaded too late in the view controller lifecycle.
@IBDesignable
open class IBIncludedSubThing: UIView, IBIncludedThingLoadable {
	...
	/// Locates the view controller with this view inside of it (depth first, since in a hierarchy 
	///	of view controllers the view would likely be in all the parentViewControllers of its view controller).
...
extension IBIncludedThingLoadable {
	...
	public func getController(inBundle bundle: Bundle) -> UIViewController? {
		...
		if let storyboardName = self.includedStoryboard {
			...
			foundController = sceneId.isEmpty
				? storyboardObj.instantiateInitialViewController()
				: storyboardObj.instantiateViewController(withIdentifier: sceneId)
	...
	/// Internal: inserts the included controller into the view hierarchy 
	///	(this helps trigger correct view hierarchy lifecycle functions)
	...
	/// Makes this nesting invisible by removing any background on IBIncludedThing 
	///	and sets constraints so included content fills IBIncludedThing.
	...
	/// parentController is only necessary when the storyboard has had no previous storyboard, 
	///	and so is missing an included controller (and its parent).
