...
//    }
    ...
    public func initContainer(isConfineToMemoryStore: Bool = false) {
        ...
        guard let storeUrl = FileManager.default.urls(
            ...
            in: .userDomainMask).first?.appendingPathComponent("\(storeName).sqlite"
        ) else {
        ...
        persistentContainer.loadPersistentStores(completionHandler: { (_, error) in // storeDescription
...
extension SimpleCoreDataManageable {
        ...
        persistentContainer.performBackgroundTask { moc in
            ...
            autoreleasepool {
                if let coreItem = moc.object(with: item.objectID) as? T {
                    ...
                    //     fetchRequest.predicate = NSPredicate(format: "(%K == %@)", #keyPath(OtherEntity.id), id)
                    // }
    ...
    public func deleteOne<T: NSManagedObject>(
        ...
        return deleteAll { (fetchRequest: NSFetchRequest<T>) in
            fetchRequest.predicate = NSPredicate(
                format: "(%K == %@)",
                #keyPath(NSManagedObject.objectID),
                item.objectID
            )
        }
    }
    ...
    public func deleteAll<T: NSManagedObject>(
        ...
        persistentContainer.performBackgroundTask { moc in
            ...
            guard let fetchRequest = T.fetchRequest() as? NSFetchRequest<T> else { return }
            alterFetchRequest(fetchRequest)
            guard let deleteFetchRequest = fetchRequest as? NSFetchRequest<NSFetchRequestResult> else { return }
