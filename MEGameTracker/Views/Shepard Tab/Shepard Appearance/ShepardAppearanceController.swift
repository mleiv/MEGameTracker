...
final class ShepardAppearanceController: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate, UIGestureRecognizerDelegate, Spinnerable {
...
//MARK: Variables
    ...
    @IBAction func ME2CodeChanged(_ sender: UITextField) {
        ...
        var appearance = Shepard.Appearance(ME2CodeField.text?.uppercased() ?? "", fromGame: game23SliderChoice, withGender: currentGender)
    ...
    func setup() {
        ...
        if !didSetup { // first time
            ...
            ME2CodeField.text = currentGame == .game2 ? pendingAppearanceGame2?.format() : pendingAppearanceGame3?.format()
        }
...
//            print("laying out \(oldAttributesTableViewHeight) \(attributesTableViewHeight) \(attributesTableView.frame)")
...
//            print("not laying out \(oldAttributesTableViewHeight) \(attributesTableViewHeight) \(attributesTableView.frame)")
        }
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: "Appearance Slider") as? ShepardAppearanceSliderCell {
    ...
    func setupSliderCell(_ cell: ShepardAppearanceSliderCell, indexPath: IndexPath) {
        ...
        guard let attribute = Shepard.Appearance.attributeGroups[currentGender]?[group]?[(indexPath as NSIndexPath).row] else {
            return
        }
        cell.define(
            ...
            notice: pendingAppearanceGame1?.notices[attribute]?.stringValue ?? Shepard.Appearance.defaultNotices[attribute]?.stringValue,
