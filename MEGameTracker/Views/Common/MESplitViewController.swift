...
public protocol MESplitViewController: class {
	...
	func performChangeableSegue(
		_ identifier: String,
		sender: AnyObject!,
		ferriedSegue: @escaping FerriedPrepareForSegueClosure
	)
...
extension MESplitViewController where Self: UIViewController {
	...
	public func performChangeableSegue(
		_ identifier: String,
		sender: AnyObject!,
		ferriedSegue: @escaping FerriedPrepareForSegueClosure
	) {
	...
	public func internalOpenDetailStoryboard(_ storyboard: String, sceneId: String?) {
		...
		if let controller = (detailPlaceholder?.includedController as? UINavigationController)?.visibleViewController {
			...
			// attach close button
			let closeButton = MESplitViewControllerBarButtonItem(
				title: "Close",
				style: .done,
				actionHandler: { [weak self] in self?.closeDetailStoryboard() }
			)
