...
final public class NotesView: SimpleArrayDataRow {
    ...
    func openNote(_ note: Note?, sender: UIView?) {
        ...
        DispatchQueue.global(qos: .background).async {
            ...
            // load editor
            if let editorNavigationController = UIStoryboard(name: "NotesEditor", bundle: Bundle(for: type(of: self))).instantiateInitialViewController() as? UINavigationController,
    ...
    override func hideEmptyView() {
        ...
        borderBottom = _borderBottom == true && !notes.isEmpty
    }
}

extension NotesView { // :UITableViewDelegate
    
    public func tableView(_ tableView: UITableView, commitEditingStyle editingStyle: UITableViewCellEditingStyle, forRowAtIndexPath indexPath: IndexPath) {
