...
//        tableView.endUpdates()
    ...
    func setupTable() {
        ...
        tableView.register(UINib(nibName: "ConversationRewardsDetailRow", bundle: bundle), forCellReuseIdentifier: "ConversationRewardsDetailRow")
...
//        }
    ...
    func saveConversationRewardsChoice(id: String, on: Bool) {
    ...
    internal func startListeners() {
        ...
        _ = Mission.onChange.subscribe(on: self) { [weak self] changed in
            if self?.mission?.id == changed.id,
                ...
                // We have to listen and change mission so that when save is called on each change, it is against the most recent copy of mission (otherwise only the last change is saved).
...
//    }
    ...
    override public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: "ConversationRewardsDetailRow") as? ConversationRewardsDetailRow {
