...
extension Styles {
    ...
    public static func applyStyle(_ style: String, toString text: String, inAttributedString attributedText: NSMutableAttributedString) -> NSAttributedString {
    ...
    public static func applyStyle(_ style: String, toAttributedText attributedText: NSMutableAttributedString) -> NSAttributedString {
        let attributes = getAttributesByStyleName(style)
        ...
        switch newStyleCategory {
            case .link:
                if let regex = try? NSRegularExpression(pattern: "^[^\\.]+", options: .caseInsensitive) {
                   return regex.stringByReplacingMatches(in: oldStyle, options: NSRegularExpression.MatchingOptions(rawValue: 0), range: NSMakeRange(0, oldStyle.characters.count), withTemplate: "AltAccentColor")
                }
            default: break
        }
        return oldStyle
    ...
    public static func convertToStyleCategory(_ newStyleCategory: StyleCategory, attributedText: NSAttributedString) -> NSAttributedString {
        var newAttributedText = NSMutableAttributedString(attributedString: attributedText)
        attributedText.enumerateAttribute("identifier", in: NSMakeRange(0, attributedText.length), options: NSAttributedString.EnumerationOptions(rawValue: 0)) { (value, range, _) -> Void in
            if let oldValue = value as? String {
                let newStyle = convertStyleCategory(newStyleCategory, oldStyle: oldValue)
                newAttributedText = NSMutableAttributedString(attributedString: self.applyStyle(newStyle, toAttributedText: newAttributedText))
            }
        }
    ...
    public static func applyGlobalStyles(_ window: UIWindow?) {
        ...
        let minimumTrackImage = UIImage(named: "Slider Filled", in: bundle, compatibleWith: nil)?.resizableImage(withCapInsets: UIEdgeInsetsMake(0,5,0,5))
        let maximumTrackImage = UIImage(named: "Slider Empty", in: bundle, compatibleWith: nil)?.resizableImage(withCapInsets: UIEdgeInsetsMake(0,5,0,5))
        ...
        UITextView.appearance().linkTextAttributes = [NSForegroundColorAttributeName: Colors.linkOnWhiteColor]
