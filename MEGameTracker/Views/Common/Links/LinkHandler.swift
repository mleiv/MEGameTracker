...
public class LinkHandler: NSObject, SFSafariViewControllerDelegate {
    ...
    fileprivate func redirectPerson(parameters: [String: String]) {
        ...
        if !alwaysDeepLink &&
            ...
            DispatchQueue.main.async {
                ...
                self.pushStoryboard(StoryboardIdentifier(name: "GroupFlow", scene: "Person")) {
                    (destinationController: UIViewController) in
                    destinationController.find(controllerType: PersonController.self) {
                        (controller: PersonController) in
            ...
            switchToLinkableTab(.group, toControllerType: GroupSplitViewController.self) {
                (controller: UIViewController) in
                (controller as? GroupSplitViewController)?.deepLink(person, type: "Person")
            }
        }
        // I also need a new link type that specifies popover = true, for stuff I really just want to popover (like short messages).
    }
        ...
        if let mapId = parameters["id"], let map = Map.get(id: mapId) {
            if !alwaysDeepLink && originController?.tabBarController?.selectedIndex == MEMainTab.maps.rawValue {
                ...
                DispatchQueue.main.async {
                    ...
                    self.pushStoryboard(StoryboardIdentifier(name: "MapsFlow", scene: "Map")) {
                        (destinationController: UIViewController) in
                        destinationController.find(controllerType: MapController.self) {
                            (controller: MapController) in
                            controller.map = map
                        }
                    }
                }
                ...
                switchToLinkableTab(.maps, toControllerType: MapsController.self) {
                    (controller: UIViewController) in
                ...
                if let type = MapLocationType(stringValue: parameters["type"] ?? ""),
                   let mapLocation = MapLocation.get(id: id, type: type, gameVersion: gameVersion) {
                    ...
                    if var dataMap = DataMap(data: SerializableData.safeInit(list)) {
                        dataMap.mapLocationPoint = MapLocationPoint(x: CGFloat(mapLocationX), y: CGFloat(mapLocationY), radius: CGFloat(mapLocationRadius))
                        dataMap.inMapId = id
        ...
        if !alwaysDeepLink && originController?.tabBarController?.selectedIndex == MEMainTab.maps.rawValue,
           ...
           let map = Map.get(id: mapId, gameVersion: gameVersion) {
            ...
            DispatchQueue.main.async {
                ...
                if let controller = self.originController as? MapController, controller.map == map && controller.self.mapLocation?.isEqual(mapLocation) == true {
                    return // do nothing, here already
                }
    ...
    fileprivate func redirectMission(parameters: [String: String]) {
        ...
        if !alwaysDeepLink && originController?.tabBarController?.selectedIndex == MEMainTab.missions.rawValue {
            ...
            DispatchQueue.main.async {
                ...
                self.pushStoryboard(StoryboardIdentifier(name: "MissionsFlow", scene: "Mission")) {
                    (destinationController: UIViewController) in
                    destinationController.find(controllerType: MissionController.self) {
                        (controller: MissionController) in
            ...
            // go to maps tab and load up map, deep link style
            switchToLinkableTab(.missions, toControllerType: MissionsGroupsController.self) {
                (controller: UIViewController) in
    ...
    public func pushStoryboard(_ storyboard: StoryboardIdentifier, withClosure closure: ConfigureViewController) {
        ...
        if let controller = (storyboard.scene != nil ? storyboardObj.instantiateViewController(withIdentifier: storyboard.scene!) : storyboardObj.instantiateInitialViewController()) {
            closure(controller)
            DispatchQueue.main.async {
    ...
    public func switchToLinkableTab<T: UIViewController>(_ tab: MEMainTab, toControllerType: T.Type, withClosure closure: @escaping ConfigureViewController) {
        // go to tab
        DispatchQueue.main.async {
            ...
            if let targetController = targetController, let _ = targetController as? T {
                ...
                currentController?.find(controllerType: UIViewController.self) {
                    (destinationController: UIViewController) in
