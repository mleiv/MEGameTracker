...
//        nameLabel.isEnabled = mission?.isAvailable ?? false
        ...
        if !(mission?.isAvailable ?? false) {
            if !isCalloutBoxRow, let text = mission?.unavailabilityMessages.joined(separator: ", ") , !text.isEmpty {
                availabilityLabel?.text = text
                availabilityLabel?.isHidden = false
            }
        } else {
            if let annotationNote = mission?.annotationNote , !annotationNote.isEmpty {
