...
final public class MapController: UIViewController, Spinnerable, UIScrollViewDelegate, UIGestureRecognizerDelegate, MapImageSizable {
...
//        }
    ...
    func openMapLocationable(_ mapLocation: MapLocationable, isIgnoreAvailabilityRules: Bool) {
        guard mapLocationsList.add(location: mapLocation, isIgnoreAvailabilityRules: isIgnoreAvailabilityRules)  else { return }
    ...
    func segueToMission(_ mission: Mission) {
        ...
        linkHandler.switchToLinkableTab(.missions, toControllerType: MissionsGroupsController.self) {
...
//    }
    ...
    fileprivate func startListeners() {
        ...
        _ = Map.onChange.subscribe(on: self) { [weak self] changed in
            ...
            if let index = self?.mapLocations.index(where: { $0.id == changed.id }), var newMap = Map.get(id: changed.id) {
        ...
        _ = Mission.onChange.subscribe(on: self) { [weak self] changed in
            // catch event changes
            guard changed.object == nil else { return }
            if let index = self?.mapLocations.index(where: { $0.id == changed.id }), var newMission = Mission.get(id: changed.id) {
        ...
        _ = Item.onChange.subscribe(on: self) { [weak self] changed in
            // catch event changes
            guard changed.object == nil else { return }
            if let index = self?.mapLocations.index(where: { $0.id == changed.id }), var newItem = Item.get(id: changed.id) {
...
extension MapController {
    ...
    func setCheckboxImage(isExplored: Bool, isAvailable: Bool) {
        if !isAvailable {
            checkboxImageView?.image = isExplored ? MapCheckbox.disabledFilled.getImage() : MapCheckbox.disabledEmpty.getImage()
        } else {
...
extension MapController {
    ...
    func setDisclosureImage(isOpen: Bool) {
        verticalDisclosureImageView?.image =  UIImage(named: isOpen ? VerticalDisclosureImage.Open.rawValue : VerticalDisclosureImage.Closed.rawValue)
    }
