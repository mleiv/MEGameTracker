...
//    public var onClick: ((MapLocationButtonNib)->Void)?
    ...
    public func set(location: MapLocationable, isShowPin: Bool = false) {
        ...
        pinColor = (location as? Item)?.itemDisplayType?.color ?? Styles.Colors.tintColor
    ...
    public func size(isForce: Bool = false) {
        ...
        let useZoomScale: CGFloat = max(zoomScale, 1.0) // only scale if we are drawing the shape, otherwise stick to map dimensions given.
