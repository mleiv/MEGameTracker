...
final public class MapsController: UITableViewController, Spinnerable {
    ...
    func setupMapCalloutRow(_ indexPath: IndexPath, cell: MapRow) {
        if (indexPath as NSIndexPath).row < searchedMapLocations[.map]?.count ?? 0, let callout = searchedMapLocations[.map]?[(indexPath as NSIndexPath).row] as? Map {
            _ = cell.define(
                map: callout,
                origin: self,
                isCalloutBoxRow: false,
    ...
    func setupMissionCalloutRow(_ indexPath: IndexPath, cell: MissionRow) {
        if (indexPath as NSIndexPath).row < searchedMapLocations[.mission]?.count ?? 0, let callout = searchedMapLocations[.mission]?[(indexPath as NSIndexPath).row] as? Mission {
            _ = cell.define(mission: callout, origin: self)
        }
    }
    
    func setupItemCalloutRow(_ indexPath: IndexPath, cell: ItemRow) {
        if (indexPath as NSIndexPath).row < searchedMapLocations[.item]?.count ?? 0, let callout = searchedMapLocations[.item]?[(indexPath as NSIndexPath).row] as? Item {
...
//        }()
    ...
    override public func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        segue.destination.find(controllerType: MapSplitViewController.self) {
...
extension MapsController {
    ...
    func setupSearch() {
        ...
        resultsController.tableView?.register(UINib(nibName: "MissionRow", bundle: bundle), forCellReuseIdentifier: "MissionRow")
        ...
        searchManager = MESearchManager(controller: self, tempSearchBar: tempSearchBar, resultsController: resultsController, searchData: searchMapLocations, closeSearch: closeSearch)
