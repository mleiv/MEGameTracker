...
final class PersonTests: MEGameTrackerTests {
	...
	let kaidanJson = "{\"id\": \"S1.Kaidan\",\"name\": \"Kaidan Alenko\",\"description\": \"Staff Lieutenant aboard the Normandy, Kaidan is the first addition to Shepard\'s squad. Kaidan\'s biotic abilities came at a high cost, emotionally and physically, and the older L2 implants cause him constant migraines.\",\"personType\": \"Squad\",\"isFemaleLoveInterest\": true,\"race\": \"Human\",\"profession\": \"Sentinel\",\"organization\": \"Systems Alliance\",\"photo\": \"http:\\/\\/urdnot.com\\/megametracker\\/app\\/images\\/Game1\\/1Kaidan.png\",\"voiceActor\": \"Raphael Sbarge\",\"relatedLinks\": [\"https:\\/\\/masseffect.wikia.com\\/wiki\\/Kaidan_Alenko\"],\"relatedMissionIds\": [\"M1.Prologue\", \"M1.Virmire.10\"],\"relatedDecisionIds\": [\"D1.Kaidan\", \"D1.LoveKaidan\", \"D3.LoveKaidan\"],\"loveInterestDecisionId\": \"D1.LoveKaidan\",\"sideEffects\": [\"If you make bad decisions in Game 3, you are forced to kill Kaidan during [megametracker:\\/\\/mission?id=M3.2.Citadel2]: [YouTube|https:\\/\\/www.youtube.com\\/watch?v=XDnp86lUnB4].\"],\"gameVersionData\": {\"2\": {\"personType\": \"Associate\",\"isFemaleLoveInterest\": false,\"description\": \"A loyal friend and former squadmate, Kaidan has mixed feelings seeing Shepard alive but working for Cerberus on Horizon. He is suspicious that Cerberus has tampered with Shepard\'s mind.\",\"loveInterestDecisionId\": null,\"photo\": \"http:\\/\\/urdnot.com\\/megametracker\\/app\\/images\\/Game2\\/2Kaidan.png\"},\"3\": {\"description\": \"Kaidan begins Game 3 on Shepard\'s team, but whether he remains depends on how hard Shepard works to persuade him that Cerberus didn\'t change Shepard in Game 2.\",\"loveInterestDecisionId\": \"D3.LoveKaidan\",\"photo\": \"http:\\/\\/urdnot.com\\/megametracker\\/app\\/images\\/Game3\\/3Kaidan.png\"}}}"
	...
	func testLoveInterest() {
		...
		XCTAssert(liara1?.isAvailableLoveInterest == true,
			"Reported incorrect love interest availability (interest found)")
		XCTAssert(kaidan1?.isAvailableLoveInterest == false,
			"Reported incorrect love interest availability (wrong gender)")
		XCTAssert(saren1?.isAvailableLoveInterest == false,
	...
	func testGetByGameVersion() {
		...
		_ = create(Person.self, from: sarenJson)
		let liara1 = Person.get(id: "S1.Liara")
		XCTAssert(liara1?.personType == .squad,
			"Failed to load correct game version")
		let liara2 = Person.get(id: "S1.Liara", gameVersion: .game2)
		XCTAssert(liara2?.personType == .associate,
			"Failed to load correct game version")
		let liara3 = Person.getAllAssociates(gameVersion: .game2).first
		XCTAssert(liara3?.personType == .associate,
			"Failed to load correct game 2 associates")
		let saren1 = Person.get(id: "E1.Saren", gameVersion: .game2)
		XCTAssert(saren1?.isAvailable == false,
			"Failed to change game version availability")
		XCTAssert(saren1?.isAvailableInGame(.game1) == true,
			"Failed to report correct game version availability")
	}
}
