...
final class AppTests: MEGameTrackerTests {
    ...
    func testCreate() {
        ...
        XCTAssert(App.current.recentlyViewedMaps.contents.isEmpty, "Initialized recently viewed maps not empty")
        XCTAssert(App.current.recentlyViewedMissions.isEmpty == true, "Initialized recently viewed missions not empty")
    }
    ...
    func testGet() {
        ...
        App.retrieve()
        XCTAssert(App.current.game?.uuid == femShepGameUuid, "Failed to load correct game version")
    ...
    func testGetRecentlyViewed() {
        ...
        _ = create(App.self, from: appWithRecentJson)
        App.retrieve()
        XCTAssert(App.current.recentlyViewedMissions[.game1]?.contents.count == 2, "Failed to load recently viewed missions")
        XCTAssert(App.current.recentlyViewedMaps.contents.count == 1, "Failed to load recently viewed maps")
    }
    ...
    func testGetRecentlyViewedMissionsByGameVersion() {
        ...
        XCTAssert(App.current.recentlyViewedMissions[.game1]?.contents.count == 1, "Failed to set recently viewed missions")
        XCTAssert(App.current.recentlyViewedMissions[.game2]?.contents.count == 1, "Failed to set recently viewed missions")
